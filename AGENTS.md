...
* **带版本号的产物只保留最新一版**：产物文件名含版本号时（如 `fintech_report_tool-1.0.13-6d0b5dc80d0f.tar.gz`），新版生成后**立即删除旧版本号文件**（含 `git rm`），目录里只留最新版 + 一个无版本号的通用名（打包源/导入用）。历史版本靠 git 历史追溯，不靠工作区堆积。
* 分发态调用（去掉对本地路径的依赖）：
