...
| 动作性质 | 选用 | 理由 |
|---|---|---|
| **输入框填写 / 单选 / 勾选 / 下拉选中**（页面没变，原页面可见） | **after** | before 里框是空的/选项未选中，看不出动作结果；after 里已输入内容、选中态可见且原布局仍在，能回答"选对了吗/输对了吗"（值同时写进说明文字） |
| **点击引发跳转/弹层打开**（click 后紧跟 nav） | **before** | before=动作瞬间现场，红框正落在被点元素上；after 页面已变，框会指空 |
| **点击触发局部刷新**（有业务请求但不离开页面，如局部列表刷新） | **before + after 连排** | before 标"点哪"，after 展示"变成什么" |
| **提交/保存/删除确认类**（submit 或按钮文本含提交/保存/确定/登录/删除） | **after** | 展示操作结果页 |
| 其他 | **before** | 默认兜底 |

> 拿不准时自问：**after 图里还能看到红框位置吗？** 能看到且结果可见 → after；看不到（页面已换）→ before。详见 references/screenshot-selection.md（含判定伪码）。

**跳过引用**：`status=raced/failed` 的图。**零尺寸元素**（rect.w=0）没有红框，只有元素文字描述——引用时配文字说明位置。

## 输出物与排版（模板文件为准，保证逐字一致）

输出目录 = `<session_dir>/../guide-output/<文档名>/`（**自动命名**：用户指定名优先；否则从 nav URL 主题+核心动作提炼，如 `监控套件创建与删除`）。只**拷贝被引用的截图**到输出目录（session 原目录不动）。

**排版一致性机制**：guide.md 的结构**不靠临场发挥**——`references/guide-template.md` 是唯一排版真源，含文档级模板、阶段章/小节模板块（3 种变体）、逐条自查硬规则。生成 = 填 «占位符» + 复制模板块，禁止自创结构。API 报告同理用 `references/api-report-format.md` 的七字段模板。

### guide.md 结构速览（细节见模板文件）

```
# <主题> · 图文操作指引        ← 文档头三行元信息（模板固定）
## 一、<阶段名>               ← 阶段 = nav 大节点
### 1.1 <动词短语>            ← 小节三要素：标题/一句说明/图
...
# 附录：接口概览表            ← 仅业务接口，详析指向 api-report.md
```
...
5. **照 guide-template.md 模板写 guide.md**（填占位符+复制模板块，引用相对路径 `screenshots/...`）
