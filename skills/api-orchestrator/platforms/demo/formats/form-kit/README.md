...
- 权威源（两处）：
  ...
  · 表单设计器保存链（容器/控件/属性面板）：**运行中前端包** `bricks/itsc-form-management/1.100.7`
    （2026-08-15 实拉 `https://<前端>/next/sa-static/-/bricks/itsc-form-management/1.100.7/dist/index.<hash>.js`
    反编译——bootstrap 里没有设计器逻辑，它在编译后的 npm brick 包里；包清单在
    `sa-static/micro-apps/v3/itsc-union/<ver>/bootstrap-union.<hash>.json` 的 brickPackages）
    · getFormData 三层链：validateSection（容器 12 规则）→ validateField（控件 9 规则，
      跳过 cmdb实例操作容器）→ validateAllForm（属性面板 schema，Tab 页签 2 规则静态可测）
    · 🔴后端【不拦任何设计器规则】（2026-08-15 探针实测：label>20 / modelField 重复/空
      等 update 全部 code=0 放行）——绕过前端直调 API 的编排必须自跑 E 类校验兜底，
...
- 规则清单（rule_id 与前端挂载点对应，详见文件头注释）：
  ...
  · E 设计器保存链（前端包反编译）：
    ...
    E-P2~P3 属性面板：Tab页签≥1个/页签标题 ^\S{1,128}$
