...
> 「探索够不够」不能靠 LLM 主观感觉——默认感觉偏「够了」。改用可枚举的**知识维度**，逐项扫过才算完。这是对「证据纪律」的操作化：证据纪律说「不许臆测」，本标尺说「什么叫探索完了」。
>
> ⚠️ **与技术栈 / 输入形态解耦**：onboarding 可接入任意平台，输入可能是契约（OpenAPI/EAML）、抓包、接口文档、或后端源码（Go/Java/Python…任意语言）。本标尺按「要接哪些知识」立维度，**不假设一定是 Go 后端或有源码**——每个维度的「证据来源」随输入形态选（下表只是举例）。

要接进 platforms 的知识无非四类，每类都要从「权威来源」拿到证据：

| 知识维度 | 有源码时的证据来源（举例，任意后端语言） | 无源码（契约/文档/抓包）时的证据来源 |
|---|---|---|
| ① 端点面 | 路由注册处（Web 框架路由，如 gin/echo 的 `.GET/.POST`、路由表） | 契约 path、接口文档、抓包（HAR/curl） |
| ② 字段结构 | 契约引用的 model/struct 定义（字段 + tag + 校验规则） | 契约 schema、在线 API schema、实测响应结构 |
| ③ 值域（enum/正则/默认） | 枚举全量源（fixtures/testdata/枚举接口/常量定义） | 契约 enum、文档取值表、实测枚举值 |
| ④ 行为/副作用 | 校验/业务逻辑（validator/service/中间件的运行时检查） | 文档行为说明、实测（e2e） |

**每个维度只允许三种状态**，探索汇报时逐项标注：

| 状态 | 含义 | 处置 |
|---|---|---|
| 已覆盖 | 有证据（file:line / 契约字段 / 文档 / 真实样例 / 实测） | 可直接进 platforms |
| gap | 查过、确实查不到 | 标「未知—待捕获」，是有效产出 |
| 漏查 | 还没查 | **不许进 platforms**——要么回炉查，要么改判 gap |

**核心禁令**：探索结束不许存在「既没覆盖、也没标 gap、也没列漏查」的静默项——那是「够用就行」的藏身处。把「漏查」误当「已确认」或「gap」，是臆测的根因。

> 有源码时四个维度全开；只有契约/文档时，①②③ 的证据退化到「契约/文档本身 + 实测补缺」——**契约/文档没写的就标 gap，别假设「有源码可查」去脑补一个来源**。
...
- **产出「契约对象/端点清单」**：这是步 3 结束后的逐项核对基准——对照它判断每个端点/字段是「已覆盖」「gap」「漏查」。清单本身也是步 4 写 api-cli 清单的骨架。
...
#### 探索汇报硬约束（防 agent「够用就行」）

派 Explore 子代理时，要求它返回**三档结论 + 证据表**，禁止自由文本结论（自由文本里「推测」会被 LLM 当「实锤」采信）：

```yaml
findings:
  - claim: <结论句，一句一个>
    verdict: 实锤 | 推测 | 未查      # 强制三档，禁止只有「看起来对」的口气
    evidence: <file:line / 真实样例 / 实测，未查则留空>
uncovered:                            # 强制输出「我覆盖到了什么 / 没覆盖到什么」
  - <契约端点或字段，本 agent 没定位到的>
```

**采纳纪律（关键，否则三档白标）**：
- `实锤` → 可直接写进 platforms，但要保留 `evidence` 作为 `source`。
- `推测` → **禁止写进 platforms**。回炉再查（换搜索角度/读源码/实测），查不到改判 gap；查实了升级为实锤。
- `未查` → 列入 `uncovered`，汇总后统一判：要么回炉补查，要么显式标 gap。
- 多个 agent 结论冲突时，以「能给出 file:line / 实测」的那个为准；都无证据 → 标 gap，不投票选多数。

> 为什么非这样不可：LLM 默认倾向「搜到几处 → 觉得够了 → 采信」。三档表把「推测」从结论里剥出来，`uncovered` 把「没搜到」显式列出来——两者都不许静默溜进 platforms。

#### 漏查 vs gap 检查点（步 3 结束必过，防「漏查当 gap」）

对照步 2 的「契约对象/端点清单」+ 四个知识维度（端点面/字段结构/值域/行为副作用），逐项核对：

- 每个端点/字段/枚举值域，**都有权威证据了吗？**
- 没有的 → 分清楚是「**查过查不到**」（= gap，标「未知—待捕获」）还是「**还没查**」（= 漏查，回炉）。
- **漏查不许进产出**；只有 gap 才允许「标了不查」。
- 自检：四维度每一项的状态都在「已覆盖 / gap / 漏查」三态里显式落位，无「静默未提」项 → 才算探索完。

> 「够用就行」的本质就是把漏查静默当成了已确认；这个检查点逼它显式暴露。
...
> **值域断言纪律**：`enum` / `pattern` / `ref` / 非平凡 `default` 是**值空间断言**，最容易被字段名/直觉臆测（反例：`foreignObjectId` 猜填模型 id）。写这些时自问「这个值空间有 file:line / 真实样例 / 实测证据吗？」没有 → 降级标 gap，别凭「看起来合理」编。若某 field 的值域来源与 object 的 `source:` 不同（如枚举来自 fixture 文件、而 object 结构来自 struct），在该 field 上补 `source:` 覆盖 object 级（asset-schema 支持 field 级 source，覆盖而非重复）。
