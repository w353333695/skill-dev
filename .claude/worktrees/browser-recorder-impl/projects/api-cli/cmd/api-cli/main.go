...
// 破坏后续 cobra 解析。这里只认 --spec/--mcp/-- 三个名字，遇到任何其他 token
// （无论 flag 还是子命令）立即停止，把剩余原样交还 cobra。
//
// 关键：全局 persistent flag（--endpoint/--insecure/--format 等）出现在子命令之前时，
// 不能由这里吞掉 —— 否则会丢 flag 本身或误吞其值（如 --endpoint backend 会只剩 backend），
// 导致 rest 残缺、cobra 回落 root help。让它们原样进 rest，由 cobra 的 TraverseChildren
// （root 已设该字段）在子命令前自行解析。
//
// 返回 (specPath, mcpMode, rest)：
//   - specPath：--spec 的值（--spec <val> 或 --spec=<val>）；未给为 ""。
//   - mcpMode：--mcp / --mcp=true 出现即为 true（修 M5：不再只认 os.Args[1]）。
//   - rest：从首个非 --spec/--mcp token 开始的全部 token，交给 cobra。
//
