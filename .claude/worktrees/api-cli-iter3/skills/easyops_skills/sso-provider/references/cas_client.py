...
class CASClient(object):
    ...
    def create_session(self, ticket, payload=None, expires=None):
        ...
        logging.info('[CAS] Creating session for ticket {}'.format(ticket))
    ...
    def delete_session(self, ticket):
        ...
        '''
        assert isinstance(self.session_storage_adapter, CASSessionAdapter)
        logging.info('[CAS] Deleting session for ticket {}'.format(ticket))
    ...
    def get_auth_token_login_url(
        ...
        logging.info('[CAS] AuthToken: {}'.format(auth_token))
        url = self._get_auth_token_login_url(
            auth_token=auth_token,
            auth_token_signature=auth_token_signature,
            service_url=service_url,
        )
        logging.info('[CAS] AuthToken Login URL: {}'.format(url))
        return url
    ...
    def get_destroy_other_sessions_url(self, service_url=None):
        ...
        logging.info('[CAS] Destroy Sessions URL: {}'.format(url))
        return url
    ...
    def get_login_url(self, service_url=None):
        ...
        logging.info('[CAS] Login URL: {}'.format(url))
        return url
    ...
    def get_logout_url(self, service_url=None):
        ...
        logging.info('[CAS] Logout URL: {}'.format(url))
        return url
    ...
    def parse_logout_request(self, message_text):
        ...
        logging.info('[CAS] LogoutRequest:\n{}'.format(
    ...
    def perform_proxy(self, proxy_ticket, headers=None):
        ...
        logging.info('[CAS] Proxy URL: {}'.format(url))
    ...
    def perform_proxy_validate(self, proxied_service_ticket, headers=None):
        ...
        logging.info('[CAS] ProxyValidate URL: {}'.format(url))
        return self._perform_cas_call(
            url,
    ...
    def perform_service_validate(
        ...
        logging.info('[CAS] ServiceValidate URL: {}'.format(url))
    ...
    def session_exists(self, ticket):
        ...
        logging.info('[CAS] Session [{}] exists: {}'.format(ticket, exists))
        return exists
    ...
    def _perform_cas_call(self, url, ticket, headers=None):
        if ticket is not None:
            logging.info('[CAS] Requesting Ticket Validation')
            response_text = self._perform_get(url, headers=headers)
            response_text = self._clean_up_response_text(response_text.encode('utf-8'))
            if response_text:
                logging.info('[CAS] Response:\n{}'.format(response_text))
                return CASResponse(response_text)
        logging.info('[CAS] Response: None')
        return None
