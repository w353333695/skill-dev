...
# ---------------- 步骤4: nginx 白名单 ----------------
# 向 tool.conf 的每个 server{} 及 cmdb*.conf 注入 allow/deny(幂等: 已注入则跳过)
step4_nginx_allow() {
  ...
  for target in $confs; do
    if rn "$node" "grep -q 'deny all;' '$target'"; then
      log_ok "[$node] $(basename "$target") 已包含白名单(deny all;), 跳过"
      continue
    fi
    backup_file "$node" "$target" || { log_warn "[$node] $(basename "$target") 备份失败"; }
    if $DRY_RUN; then log_info "[$node] [dry-run] 将向 $(basename "$target") 的每个 server{} 注入 allow/deny"; continue; fi

    # 注入方式: 拉取到本地 -> awk 在每个 server{ 行后插入 allow 块 -> 推回
    # (本地转换避免远端 python3 依赖; tool.conf 有 2 个 server{}, awk 会逐一处理)
    fetch "$node" "$target" "$work/$(basename "$target")" || { log_err "[$node] 拉取 $(basename "$target") 失败"; return 3; }
    ...
    ' "$work/$(basename "$target")" >"$work/$(basename "$target").new"
    local rc=$?
    if [ $rc -ne 0 ]; then
      log_err "[$node] $(basename "$target") 中未找到 server{ 块, 不修改"
      continue
    fi
    if push "$node" "$work/$(basename "$target").new" "$target" && rn "$node" "chown easyops: '$target' && grep -q 'deny all;' '$target'"; then
...
# ---------------- 回滚 ----------------
# 依据备份目录 manifest.tsv 按节点恢复:
#   FILE/MV  -> 拷回原路径
#   NEW      -> 删除新建文件
#   IPT      -> iptables-restore 恢复快照
# nginx 恢复后统一 -t 校验 + reload
pick_backup_dt() {  # pick_backup_dt <node> -> echo 最新(或指定)备份时间目录
  local node="$1" d
  if [ -n "$BACKUP_DT" ]; then
    [ -d "$(bk_dir "$node" "$BACKUP_DT")" ] || { log_err "[$node] 指定备份不存在: $BACKUP_DT"; return 1; }
    echo "$BACKUP_DT"; return 0
  fi
  d=$(ls -1 "$BK_ROOT/$node" 2>/dev/null | grep -E '^[0-9]{8}-[0-9]{6}$' | sort | tail -1)
  [ -z "$d" ] && { log_err "[$node] 无任何备份目录"; return 1; }
  echo "$d"
}
...
rollback_node() {
  ...
  local dt; dt=$(pick_backup_dt "$node") || return 3
  local bdir; bdir="$(bk_dir "$node" "$dt")"
  log_info "[$node] 使用备份: backup/$node/$dt"
  [ -f "$bdir/manifest.tsv" ] || { log_err "[$node] 缺少 manifest.tsv, 无法回滚"; return 3; }

  local nginx_touched=false
  # 跳过表头行
  while IFS=$'\t' read -r tag rpath bkfile _desc; do
    [ -z "${tag:-}" ] && continue
    [ "$tag" = "类型" ] && continue
    case "$tag" in
      FILE|MV)
        if push "$node" "$bdir/$bkfile" "$rpath"; then
          log_ok "[$node] 已恢复 $rpath"
          # MV 类: 清掉加固时 mv 产生的 .bak_hardening 残留, 保证可再次加固
          [ "$tag" = "MV" ] && rn "$node" "rm -f '${rpath}.bak_hardening'"
          case "$rpath" in */nginx/conf/conf.d/*) nginx_touched=true ;; esac
        else
